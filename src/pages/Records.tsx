...
function AdminRecords() {
  ...
  const { flash, show: showFlash, clear: clearFlash } = useFlashAlert()
  ...
  useEffect(() => {
    const t = setTimeout(async () => {
      try {
        ...
        showFlash({
          type: "error",
          title: "Error buscando expedientes",
          description: e?.message ?? "Revisá tu conexión.",
        })
      }
    }, 300)
      ...
      try {
        ...
        if (added > 0) {
          showFlash({
            ...
            description: `Se agregaron ${added} cita(s) completada(s) al expediente.`,
          })
        }
      } catch (e: any) {
        showFlash({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      try {
        ...
        const added = res?.synced?.added ?? 0
        if (added > 0) {
          showFlash({
            type: "success",
            title: "Expediente actualizado",
        ...
        <Card className="glass-card rounded-2xl">
          ...
          <CardContent className="space-y-4">
            ...
            {loading ? (
              ...
              <CustomAlert
                type="info"
                title="Aún no hay un expediente vinculado"
                description={
                  "Cuando tengas tu primera cita, recordá pedirle a la doctora que vincule tu cuenta a tu expediente."
                }
              />
            ) : (
              <>
...
function CreateRecordDialog({
  ...
  useEffect(() => {
    if (!err) return
    const t = window.setTimeout(() => setErr(null), 3500)
  ...
  return (
    ...
    >
      <DialogContent className="sm:max-w-xl rounded-2xl p-6">
        ...
        {err ? (
          <CustomAlert type="error" title="Error" description={err} onClose={() => setErr(null)} />
        ) : null}
